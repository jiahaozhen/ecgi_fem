...
def build_tau_in(f_space: functionspace, condition: Type[Callable], ischemia=False):
    ...
    if ischemia:
        coords = f_space.tabulate_dof_coordinates().T
        tau_in_smooth = condition(TAU_IN_ISCHEMIA, TAU_IN)(coords)
...
def build_D(f_space: functionspace, condition: Type[Callable], scar=False, ischemia=False):
    ...
    if scar:
        D_smooth = condition(D_VAL_SCAR, D_VAL)(coords)
        D.x.array[:] = D_smooth
    elif ischemia:
        D_smooth = condition(D_VAL_ISCHEMIA, D_VAL)(coords)
        D.x.array[:] = D_smooth
    else:
        D.x.array[:] = D_VAL
...
def build_Mi(domain: Mesh, condition: Type[Callable], sigma_i=0.4, scar=False, ischemia=False):
    ...
    if ischemia:
        mask = condition(1.0, 0.0)(coords)  # 连续0~1 掩码
...
def build_M(domain: Mesh, cell_markers, condition: Type[Callable], multi_flag, sigma_i=0.4, sigma_e=0.8, sigma_t=0.8, scar=False, ischemia=False):
    ...
    def rho2(x):
        tensor = np.eye(tdim) * (sigma_e + sigma_i)
        n_dofs = x.shape[1]
        values = np.tile(tensor.reshape(-1, 1), n_dofs)
        if ischemia:
            mask_local = condition(1.0, 0.0)(x)  # 使用局部掩码
            factor_local = 1 - (1 - (ME_ISCHEMIA_FACTOR + MI_ISCHEMIA_FACTOR) / 2) * mask_local
        elif scar:
            mask_local = condition(1.0, 0.0)(x)
            factor_local = 1 - (1 - (ME_SCAR_FACTOR + MI_SCAR_FACTOR) / 2) * mask_local
        else:
            factor_local = np.ones(n_dofs)
        values *= factor_local
        return values
    def rho3(x):
