...
if __name__ == "__main__":
    ...
    activation_dict = get_activation_dict(
        case_name=case_name, mode='ENDO', threshold=40
    )
    ...
    activation_1 = extract_data_from_function(
        activation_f, get_function_space(mesh_file), ventricle_pts
    ).reshape(-1)
    ...
    p1 = multiprocessing.Process(
        target=plot_triangle_mesh,
        args=(ventricle_pts, ventricle_fac),
        kwargs={'point_values': activation_1, 'title': 'pde'},
    )
    p2 = multiprocessing.Process(
        target=plot_triangle_mesh,
        args=(ventricle_pts, ventricle_fac),
        kwargs={'point_values': activation_2, 'title': 'ecgsim'},
    )
    p1.start()
    p2.start()
