...
phi_1_exact, _ = compute_phi_with_v_timebased(v_exact, V2, a2, a4)
phi_1_exact = np.where(phi_1_exact[0] < 0, -tau, tau)

#initial phi_1 phi_2
phi_0 = np.full(phi_1.x.array.shape, tau/2)
phi_1.x.array[:] = phi_1_exact
phi_2.x.array[:] = phi_0
...
for timeframe in range(time_total):
    ...
    end_time = time.time()
    cc = np.correlate(v_exact[timeframe], v_result[timeframe][0, 1])
    ...
    print('correlation coefficient:', cc[-1])
